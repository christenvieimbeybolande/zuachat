...
class _FollowersPageState extends State<FollowersPage> {
  ...
  Future<void> _loadFollowers() async {
    ...
    final result = await fetchFollowers(widget.userId);
    if (result["ok"] == true) {
      setState(() => followers = result["followers"]);
    }
  ...
  Future<void> _toggleFollow(int id) async {
    final index =
        followers.indexWhere((u) => u['id'].toString() == id.toString());
    if (index == -1) return;
